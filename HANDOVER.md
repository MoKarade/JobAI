...
| **Base de données** | Neon (`us-east-2`), migration `0000` **appliquée**. ⚠️ **La migration `0001` (table `villes`) N'EST PAS ENCORE APPLIQUÉE** — `npm run db:migrate` à lancer. Jeu de départ **chargé**. Connexion paresseuse : le module s'importe au build sans `DATABASE_URL`, l'erreur ne part qu'à la première requête réelle. ⚠️ Le mot de passe initial a été exposé en conversation le 2026-07-28 et **doit avoir été régénéré** — à confirmer. |
...
- [ ] ⚠️ **Appliquer la migration `0001` (`npm run db:migrate`)** — table `villes`, sans
      laquelle l'onglet Carte reste vide. Puis, une fois en ligne, **cliquer une fois sur
      « Situer les villes »** : c'est le seul vrai signal que le géocodage fonctionne (le
