  ...
  try {
    ...
    try {
      ...
      if (await reserverPasse(db, CLE_DISTANCES, DELAI_MESURE_AUTO_MS, new Date())) {
        ...
        localisation = m.ok
          ? `${m.placees} placée(s) au centre-ville, ${m.villesRattrapees} ville(s) rattrapée(s), ${m.situees} située(s), ${m.adressesRattrapees} adresse(s) récupérée(s), ${m.bornesMesurees} borne(s) mesurée(s), ${m.mesurees} mesurée(s)`
          : `refusée : ${m.erreur}`;
